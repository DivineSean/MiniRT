...
/*   Updated: 2023/11/06 21:35:56 by markik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector	sphere_checker(t_obj *data, t_cyin in, t_vector l_d)
{
	t_vector	o_c;
	...
	if (data->checkboard)
		...
		o_c = data->sp[in.i_sphere]->o_c_bumb;
	else
...
t_vector	result_base_on_type(t_obj *data, t_vector l_d, t_cyin in, int type)
{
	...
	if (type == 1)
		...
		o_c = sphere_checker(data, in, l_d);
	else if (type == 4)
		o_c = making_new_vector(data->cn->r, data->cn->g, data->cn->b);
	uac = making_new_vector(uac.x / 255.0, uac.y / 255.0, uac.z / 255.0);
	o_c = making_new_vector(o_c.x / 255.0, o_c.y / 255.0, o_c.z / 255.0);
